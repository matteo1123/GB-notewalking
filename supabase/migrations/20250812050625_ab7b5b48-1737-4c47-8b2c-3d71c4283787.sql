...
CREATE POLICY IF NOT EXISTS "Public can read public exercises"
ON public.exercises
FOR SELECT
USING (is_public = true);

-- Allow owners to read their own private exercises
CREATE POLICY IF NOT EXISTS "Owners can read own exercises"
ON public.exercises
FOR SELECT
USING (auth.uid() = created_by);

-- Only owners can insert (created_by must match user)
CREATE POLICY IF NOT EXISTS "Owners can insert exercises"
ON public.exercises
FOR INSERT
WITH CHECK (auth.uid() = created_by);

-- Only owners can update/delete their own rows
CREATE POLICY IF NOT EXISTS "Owners can update own exercises"
ON public.exercises
FOR UPDATE
USING (auth.uid() = created_by);

CREATE POLICY IF NOT EXISTS "Owners can delete own exercises"
ON public.exercises
FOR DELETE
USING (auth.uid() = created_by);
